...
conso_alim_par_equipe <- conso_alim_par_entretien |>
	dplyr::filter(periode_rapport == TRUE) |>
	dplyr::group_by(supervisor) |>
	dplyr::summarise(
    n = dplyr::n(),
    minimum = fivenum(n_prod_alim, na.rm = TRUE)[1], 
    lower_hinge = fivenum(n_prod_alim, na.rm = TRUE)[2], 
    median = fivenum(n_prod_alim, na.rm = TRUE)[3], 
    upper_hinge = fivenum(n_prod_alim, na.rm = TRUE)[4], 
    maximum = fivenum(n_prod_alim, na.rm = TRUE)[5]
  ) |>
	dplyr::ungroup()

conso_alim_ensemble <- conso_alim_par_entretien |>
	dplyr::filter(periode_rapport == TRUE) |>
	dplyr::summarise(
    n = dplyr::n(),
    minimum = fivenum(n_prod_alim, na.rm = TRUE)[1], 
...
conso_alim_tendance_par_equipe <- conso_alim_par_entretien |>
  # calculer la médiane par superviseur et par semaine
  dplyr::group_by(supervisor, semaine_entretien) |>
  dplyr::summarise(n_prod_alim = median(n_prod_alim, na.rm = TRUE)) |>
  dplyr::ungroup() |>
  # ordonner la base par superviseur et par semaine
  dplyr::arrange(supervisor, semaine_entretien) |>
  # créer une liste de mesures ordonnées par date pour chaque superviseur
  ...
  # ensuite, en extrayant la mesure pour en faire une colonne liste
  dplyr::group_by(supervisor) |>
  tidyr::nest(tendances_df = c(semaine_entretien, n_prod_alim)) |>
  dplyr::mutate(tendance = purrr::map(tendances_df, ~ .x$n_prod_alim)) |>
  dplyr::ungroup() |>
  dplyr::select(supervisor, tendance)

conso_alim_tendance_ensemble <- conso_alim_par_entretien |>
  # calculer la médiane par semaine
  dplyr::group_by(semaine_entretien) |>
  dplyr::summarise(n_prod_alim = median(n_prod_alim, na.rm = TRUE)) |>
  dplyr::ungroup() |>
...
conso_alim_stats <- dplyr::bind_rows(
  conso_alim_ensemble, conso_alim_par_equipe
)

conso_alim_tendance <- dplyr::bind_rows(
  conso_alim_tendance_par_equipe, conso_alim_tendance_ensemble
)

conso_alim_tbl <- conso_alim_stats |>
	dplyr::left_join(conso_alim_tendance, by = "supervisor")
