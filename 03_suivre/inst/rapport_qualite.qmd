...
<details>
  <summary>ℹ️ Détails du tableau</summary>
  
- Indicateur : Nombre de membres du ménage.
- Source : nombre de noms renseignés dans `NOM_PRENOMS`.
...
- Indicateur : Nombre de produits alimentaires consommées.
- Source : nombre de produits présent dans le roster du module 7B.
...
- Indicateur : Nombre de produits non-alimentaires consommées.
- Source : questions oui/non des modules 9B à 9F

</details>

```{r}
#| label: consommation non-alimentaire

conso_non_alim_par_entretien <- equipe_enqueteur_entretien |>
  dplyr::left_join(
    dfs_filtrees$menages,
    by = c("interview__id", "interview__key")
  ) |>
	dplyr::mutate(
    n_prod_non_alim = rowSums(
      dplyr::pick(dplyr::matches(
        "(s09Bq02__|s09Cq02__|s09Dq02__|s09Eq02__|s09Fq02__)"
      )),
      na.rm = TRUE
    )
  ) |>
...
creer_tableau_reactable(
  df = conso_non_alim_par_entretien,
  var = n_prod_non_alim,
  theme = reactable_style,
  language = reactable_fr
)
...
- Indicateur : % de ménages qui pratique l'agriculture.
- Source : question `s16Aq00`
...
# calculer l'indicateur
agric_par_entretien <- equipe_enqueteur_entretien |>
  dplyr::left_join(
    dfs_filtrees$menages,
    by = c("interview__id", "interview__key")
  ) |>
...
- Indicateur : % de ménages qui pratique l'élevage.
- Source : question `s17q00`
...
# calculer l'indicateur
elevage_par_entretien <- equipe_enqueteur_entretien |>
  dplyr::left_join(
    dfs_filtrees$menages,
    by = c("interview__id", "interview__key")
  ) |>
	dplyr::mutate(
    pct_elevage = (s17q00 == 1),
    pct_elevage = pct_elevage * 100
  ) |>
	dplyr::select(
    supervisor, interviewer,
    interview__id, interview__key,
